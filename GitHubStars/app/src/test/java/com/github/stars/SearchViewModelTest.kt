...
import com.github.stars.presentation.viewmodel.SearchStarsViewModel
...
import com.github.stars.repository.services.SearchStarsRepository
import com.github.stars.usecase.SearchStarsUseCase
...
class SearchViewModelTest {
    ...
    @Mock
    lateinit var mockedRepository: SearchStarsRepository
    @Mock
    lateinit var searchUseCase: SearchStarsUseCase
    @Mock
    ...
    lateinit var viewModel: SearchStarsViewModel
    ...
    fun setUp() {
        ...
        mockedRepository = Mockito.mock(SearchStarsRepository::class.java)
        searchUseCase = Mockito.spy(SearchStarsUseCase(mockedRepository))

        viewModel = SearchStarsViewModel(context, Dispatchers.Unconfined, searchUseCase)
    ...
    fun getSearch_SUCCESS() = runBlocking {
        Mockito.`when`(
            mockedRepository.getSearchStarsList("language:kotlin", "stars", 1)
        ).thenReturn(
            Resource.Success(SEARCH_STARS.items)
        )

        viewModel.getSearchStars("language:kotlin", "stars", 1)
    ...
    fun getSearchLoadMore_SUCCESS() = runBlocking {
            Mockito.`when`(
                mockedRepository.getSearchStarsList("language:kotlin", "stars", 2)
            ).thenReturn(
                Resource.Success(SEARCH_STARS.items)
            )
